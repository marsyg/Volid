...
export const getFiles = query({
    ...
    handler: async (ctx, args) => {
        ...
        if(project._id !== identity.subject) throw new Error('Unauthorized');
...
export const createFiles = mutation({
  args: {
    ...
    handler: async (ctx, args) => {
        ...
        const existingFile = files.find(f => f.name === args.name && f.parentId === args.parentId);
...
export const renameFile = mutation({
  ...
  handler: async (ctx, args) => {
      ...
      const files = await ctx.db.query('files')
          ...
          .collect();
      const existingFile = files.find(f => f.name === args.name && f.parentId === args.parentId && f.type === args.type);
      if (existingFile) throw new Error('File already exists');
...
export const updateFile = mutation({
    ...
    handler: async (ctx , args) => {
        ...
        if (project.ownerId !== identity.subject) throw new Error('Unauthorized');
        await ctx.db.patch(args.fileId, { content: args.content, updatedAt: Date.now() });
...
export const createFolder = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const files = await ctx.db.query("files")
            .withIndex("by_project_parent", (q) => q.eq("projectId", args.projectId).eq("parentId", args.parentId))
            .collect();
        const existingFile = files.find((f) => f.name === args.name && f.type === "folder");
