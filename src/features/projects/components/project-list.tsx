...
const ProjectItem = ({ data }: {
    ...
    return <Link href={`/projects/${data._id}`}
        ...
        <span className='text-xs  text-muted-foreground group-hover:text-foreground/60 transition-colors'>{
            formatTimeStamp(data.updatedAt)}</span>
    </Link>
}
...
const ContinueCard = ({ data }: {
    ...
    return <div className='flex flex-col gap-2'>
        ...
        <Button variant='outline' asChild className='h-auto items-start justify-start p-4 background  border  rounded-none  flex flex-col gap-2'>
