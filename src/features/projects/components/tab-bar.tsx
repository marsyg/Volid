...
const Tab = ({ label, isActive, onClick }: { label: string, isActive: boolean, onClick: () => void }) => {
    return (
        <div className="">
            <button onClick={onClick} className={cn("flex px-2 items-center cursor-pointer  text-muted-foreground border-r hover:bg-accent/30", isActive ? "bg-background text-foreground" : "")}>
                {label}
            </button>
...
const TabBar = ({ activeTab, setActiveTab }: { activeTab: "code" | "preview", setActiveTab: (tab: "code" | "preview") => void }) => {
    return (
        <nav className="flex justify-self-auto  items-center border-b">
            <Tab label="code" isActive={activeTab === "code"} onClick={() => setActiveTab("code")} />
